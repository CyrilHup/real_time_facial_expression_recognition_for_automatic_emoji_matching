...
\begin{itemize}
    ...
    \item 7 classes d'émotions équilibrées (sauf \textit{Disgust})
\end{itemize}
...
\begin{enumerate}
    ...
    \bottomrule
    \end{tabular}
    \caption{Distribution des classes dans FER2013. \textit{Disgust} est très sous-représentée.}
    \end{table}
...
\begin{itemize}
    ...
    \item Possibilité d'utiliser des \textit{soft labels} (distribution de probabilités)
    \item Ajout de la classe \textit{Contempt} (mépris)
...
\begin{itemize}
    \item \textbf{Stabilité} : Le lissage temporel élimine le \emph{flickering}
